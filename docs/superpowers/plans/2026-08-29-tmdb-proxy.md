...
### Task 3: Wire the Astro endpoint + smoke-test docs
...
- Produces: a live `GET /api/tmdb` route. Task 4's client helpers call it by URL.
...
- [ ] **Step 3: Add a smoke-test section to `README.md`**

Append:

```markdown
## TMDB proxy smoke test

The proxy needs a real TMDB v4 token in `.env` (`TMDB_ACCESS_TOKEN=`). Then:

    npm run dev
    curl 'http://localhost:3000/api/tmdb?op=search&query=matrix'
    curl 'http://localhost:3000/api/tmdb?op=movie&id=603'

The first returns a TMDB search page; the second a movie object that includes a
`credits` field. Neither response contains the token.

Automated tests (`npm test`) mock `fetch` and need no token or network.
```

- [ ] **Step 4: Manual smoke test (skip if no real token)**
...
```bash
git add src/pages/api/tmdb.ts README.md
git commit -m "feat: expose /api/tmdb endpoint and document the smoke test"
```
...
| Token never in body/header/log | Task 2 (only `console.error`s status/messages, never the token); Task 3 (step 4 verifies) |
...
| Manual smoke test documented | Task 3 (step 3, README) |
| Acceptance: `npm install && npm run build` | Task 1 (step 10) |
| Acceptance: `npm test` with no network/token | Task 4 (step 4) |
| Acceptance: real-token curl returns JSON with `credits` | Task 3 (step 4) |
