...
| `commit-messages.md` | Conventional Commits: `type(optional-scope): summary`. Subject one line, imperative, ≤ ~72 chars. Optional body of 1–2 sentences summarising what and why. Keep the existing `Co-Authored-By:` trailer; never publish a session link (privacy and security). PR titles follow the same rule (a single-commit PR's title is that commit's subject verbatim). |
